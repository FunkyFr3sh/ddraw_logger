...
const char* dbg_mes_to_str(int id)
{
    ...
    case WM_APP + 116: return "WM_DISPLAYCHANGE_DDRAW";
    case WM_APP + 114: return "WM_SIZE_DDRAW";
    case WM_APP + 115: return "WM_MOVE_DDRAW";
    case  WM_APP + 113: return "WM_D3D9DEVICELOST";
    case WM_APP + 112: return "WM_WINEFULLSCREEN";
    case WM_APP + 111: return "WM_AUTORENDERER";
    case WM_APP + 117: return "WM_TOGGLE_FULLSCREEN";
    case WM_APP + 118: return "WM_TOGGLE_MAXIMIZE";
    }
...
HWND g_hwnd;
HHOOK g_wndproc_hook;
LRESULT CALLBACK CallWndProc(
	_In_ int    nCode,
	_In_ WPARAM wParam,
	_In_ LPARAM lParam)
{
	if (nCode < 0) // code != HC_ACTION || 
		return CallNextHookEx(g_wndproc_hook, nCode, wParam, lParam);

	CWPSTRUCT* m = (CWPSTRUCT*)lParam;

	DWORD uMsg = m->message;

	if (g_hwnd == m->hwnd &&
        uMsg != WM_MOUSEMOVE && uMsg != WM_NCMOUSEMOVE && uMsg != WM_NCHITTEST && uMsg != WM_SETCURSOR &&
		...
		uMsg != WM_UNICHAR && uMsg != WM_IME_CHAR && uMsg != WM_IME_KEYDOWN && uMsg != WM_IME_KEYUP && uMsg != WM_TIMER)
	{
		...
		_snprintf(
			...
			m->lParam,
			m->lParam,
			(int)(short)LOWORD(m->lParam),
			(int)(short)HIWORD(m->lParam));
